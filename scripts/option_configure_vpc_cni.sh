...
echo "  - ENABLE_PREFIX_DELEGATION: false (标准 IP 模式)"
echo ""
echo "💡 说明："
echo "  - EXTERNALSNAT=false: Pod 使用主 IP 直接访问外部，不做 NAT 转换"
echo "  - 适用场景: Pod 需要保留源 IP，或使用 VPC 内部路由"
...
kubectl exec -n kube-system ${POD_NAME} -- env | grep -E "AWS_VPC_K8S_CNI_EXTERNALSNAT|ENABLE_PREFIX_DELEGATION" || echo "Note: Variables may not show if using default values"
...
echo "  kubectl get daemonset aws-node -n kube-system -o yaml | grep -A 5 EXTERNALSNAT"
echo ""
