# 【第二篇 blog · 大纲 v2】EKS 上的 GPU 工作负载:节点、网络与高性能存储的架构实践

> **状态**:大纲 v2,基于 Kevin 的反馈调整完毕,待最终确认后扩写为正文。
> **基调**:架构解析式(每章先讲架构与设计决策,真实问题作为"设计动因"穿插)。
> **历史**:少提一句即可,不作为章节主骨架。
> **长度目标**:与第一篇相当,约 4500 字 / 约 450 行 Markdown。
> **发布渠道**:AWS 官方 Blog(注意术语严谨、行文中性,对 AWS 服务的描述避免带主观评价)。
> **主线**:GPU 工作负载的三层架构 —— **计算节点 → 网络邻近性 → 高性能存储(训练 + 推理)**。

---

## 文章定位与结构

本文是第一篇《企业级 EKS 集群生产环境配置最佳实践》的**深化篇**。第一篇搭建了能跑起来的集群,本文聚焦其中技术密度最高的一块 —— GPU 工作负载。

**三个核心问题驱动**:
1. **计算层**:GPU 节点如何高效利用多张 EFA 网卡的带宽?
2. **网络层**:跨节点训练的 allreduce 延迟如何通过拓扑感知优化?
3. **存储层**:训练场景(FSx for Lustre)和推理场景(S3 Express One Zone)的高性能存储如何选型与集成?
...
| 章节 | 主题 | 预计篇幅 |
|---|---|---|
| 引言 | GPU 工作负载的架构挑战 | ~40 行 |
| 第 1 章 | GPU 节点组:EFA 多网卡设计 | ~80 行 |
| 第 2 章 | 四种定价模式的 Launch Template 架构 | ~50 行 |
| 第 3 章 | 网络邻近性:基于 DescribeInstanceTopology 的标签化调度 | ~80 行 |
| 第 4 章 | 节点本地存储:Instance Store 与容器运行时的解耦 | ~45 行 |
| 第 5 章 | 训练场景存储:FSx for Lustre 架构 | ~55 行 |
| 第 6 章 | 推理场景存储:S3 Express One Zone + Mountpoint | ~55 行 |
| 第 7 章 | 端到端验证与最佳实践清单 | ~45 行 |
| 总结 | 能力沉淀与取舍原则 | ~25 行 |

**合计约 475 行**,含前言、图表、代码块后约 450 行成品(第一篇为对照)。
...
### 要点
- 第一篇给出了"能跑起来"的通用集群,本篇聚焦其上的 GPU 工作负载,让集群"能训练、能推理"
- 随着生成式 AI 与大模型训练在企业环境的快速落地,GPU 工作负载对**计算、网络、存储**三层都提出了超越通用节点的要求:
  - **计算层**:GPU 驱动、EFA 多网卡、Device Plugin 协同
  - **网络层**:allreduce 延迟对网络拓扑敏感,需要感知 L3 leaf 邻近性
  - **存储层**:训练需要高聚合吞吐,推理需要低延迟对象访问,两者选型截然不同
- **一句话带过历史**:GPU 节点采用 Managed Node Groups 而非 Karpenter,以便在 Launch Template 中精确控制 EFA 多网卡配置与定价模式
- **衔接第一篇**:第一篇概览了 EBS / EFS / FSx / S3 四种 CSI Driver 的接入方式,本文将深入 FSx for Lustre 与 S3 Express One Zone 这两类高性能存储在 GPU 训练/推理链路上的**选型策略、架构设计、性能优化点与已知限制**
...
> **承接第一篇**:第一篇聚焦在"把集群跑起来",未涉及跨节点训练的网络拓扑问题。对于分布式训练,节点间网络距离直接影响 allreduce 性能,本章展开这一维度的架构设计。
...
### 3.2 两种方案对比:Placement Group vs Topology Label
...
### 3.3 DescribeInstanceTopology 的返回结构
```json
{
  "Instances": [
    {
...
> **承接第一篇**:第一篇介绍了系统节点通过双 EBS + LVM 把 `/var/lib/containerd` 从根盘剥离,解决容器 I/O 与系统 I/O 竞争问题。GPU 节点继承该设计,但由于 `*d / i4g` 系列自带 Instance Store NVMe,需要额外引入一层本地 scratch 存储 `/data`,并严格区分两者用途——容器运行时必须留在 EBS,本地 scratch 才挂 Instance Store。本章展开这一设计的细节。
...
### 6.2 Standard S3 与 S3 Express One Zone 对比
...
两篇构成一套可落地、可复制、从零到 GPU 生产的完整参考实现。完整代码已在 [GitHub](https://github.com/KevinZhao/eks-cluster-deployment) 开源。

---

*本文为《企业级 EKS 实战》系列第二篇，与第一篇《企业级 EKS 集群生产环境配置最佳实践》（2026 年 5 月）接续，发布于 2026 年 6 月。*

---

*本大纲基于 `scripts/option_install_gpu_nodegroups.sh`、`scripts/option_install_csi_drivers.sh`、`scripts/topology_labeling_lib.sh` 及完整 git 历史提炼,符合 AWS 官方 Blog 的术语与行文基调。*
